...
作为一个普通本科生，这个项目对我来说不仅仅是一个作品，更是一段宝贵的成长历程。

**起点：裸机开发的迷茫**

记得刚开始接触嵌入式时，只会写简单的 GPIO 点灯、串口打印。面对这个智能手表项目，一度觉得遥不可及：
- 什么是实时操作系统？任务调度是怎么回事？
- 图形界面？LVGL 听起来好高深
- 传感器数据怎么处理？滤波算法是什么？
- 低功耗设计？完全没概念
...
1. **FreeRTOS 任务优先级** - 一开始把任务优先级都设成一样，结果系统卡顿。后来才明白，按键和屏幕刷新必须高优先级，数据保存可以放后台。

2. **页面栈的坑** - 最开始直接存 UI 函数指针 `ui_HomePage`，结果页面切换混乱。调试了一整天才发现应该存地址 `&ui_HomePage`。

3. **心率算法优化** - 官方库计算太慢，自己写峰值检测算法，调了无数次阈值和滤波参数，才达到可用的精度。

4. **低功耗调试** - 睡眠电流一直下不来，最后发现是 MPU6050 的 DMP 库初始化后功耗很高，需要修改配置寄存器。

5. **内存管理** - LVGL 和 FreeRTOS 抢内存，一开始没规划好，经常 HardFault。后来精细控制堆内存分配，才稳定下来。
...
有人说，嵌入式开发是"软硬兼施"的修行。确实如此，这个项目让我深刻体会到：

> **理论很重要，但动手实践更重要。**
> 
> 看十遍教程不如自己踩一次坑。
> 
> 代码调不通时很痛苦，但解决后的成就感无可替代。
